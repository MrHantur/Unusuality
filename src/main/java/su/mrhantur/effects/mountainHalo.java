...
public class mountainHalo implements UnusualEffect {
    ...
    public void apply(Entity player, int timer, List<Player> viewers) {
        ...
        for (int i = 0; i < points; i++) {
            ...
            // Лёгкий сияющий след
            for (Player viewer : viewers) viewer.spawnParticle(Particle.DUST, pos, 0, 0, 0, 0, glowDust);

            // Иногда — магическая искра
            if (i % 6 == 0 && timer % 8 == 0) {
                for (Player viewer : viewers) viewer.spawnParticle(Particle.CRIT, pos, 0, 0, 0, 0, 0.0);
            }

            // Иногда снежинки
            if (i % 3 == 0 && timer % 4 == 0) {
                for (Player viewer : viewers) viewer.spawnParticle(Particle.INSTANT_EFFECT, pos, 0, 0, 0, 0, 0.0);
            }
        }

        if (timer % 10 == 0) {
            for (Player viewer : viewers) viewer.spawnParticle(Particle.WAX_ON, base.clone().add(0, 0.05, 0), 1, 0, 0, 0, 0.0);
        }
    }
}
